import 'package:flutter/cupertino.dart';
import 'package:get/get.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../../shared/themes/app_theme.dart';

class ThemeService extends GetxService {
  int _currentIndex = 0;

  int get currentIndex => _currentIndex;
  String get currentName => _presets[_currentIndex].name;
  List<String> get presetNames => _presets.map((p) => p.name).toList();
  int get presetCount => _presets.length;

  @override
  void onInit() {
    super.onInit();
    _loadSavedTheme();
  }

  Future<void> _loadSavedTheme() async {
    final prefs = await SharedPreferences.getInstance();
    final savedIndex = prefs.getInt('themeIndex') ?? 0;
    if (savedIndex != _currentIndex && savedIndex >= 0 && savedIndex < _presets.length) {
      _currentIndex = savedIndex;
      AppTheme.current = _presets[savedIndex];
    }
  }

  void apply(int index) {
    if (index < 0 || index >= _presets.length) return;
    _currentIndex = index;
    AppTheme.current = _presets[index];
    SharedPreferences.getInstance().then((prefs) {
      prefs.setInt('themeIndex', index);
    });
  }

  static final List<ThemePreset> _presets = [
    const ThemePreset(
      name: '深空靛蓝',
      background: Color(0xFF0F0F1A),
      surface: Color(0xFF1A1A2E),
      surfaceLight: Color(0xFF252542),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF4F46E5),
      accentLight: Color(0xFF6366F1),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFCBD5E1),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFF4F46E5),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '极光绿',
      background: Color(0xFF0A1A0F),
      surface: Color(0xFF142E1A),
      surfaceLight: Color(0xFF1E4228),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF059669),
      accentLight: Color(0xFF10B981),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFD1FAE5),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFF059669),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '暗夜紫',
      background: Color(0xFF0F0A1A),
      surface: Color(0xFF1A142E),
      surfaceLight: Color(0xFF282042),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF7C3AED),
      accentLight: Color(0xFF8B5CF6),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFE9D5FF),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFF7C3AED),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '海洋蓝',
      background: Color(0xFF0A0F1A),
      surface: Color(0xFF14202E),
      surfaceLight: Color(0xFF1E3042),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF2563EB),
      accentLight: Color(0xFF3B82F6),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFDBEAFE),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFF2563EB),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '落日橙',
      background: Color(0xFF1A0F0A),
      surface: Color(0xFF2E1A14),
      surfaceLight: Color(0xFF42281E),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFFD97706),
      accentLight: Color(0xFFF59E0B),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF97316),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFFEF3C7),
      textMuted: Color(0xFFD4A574),
      chatBubbleMe: Color(0xFFD97706),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '樱花粉',
      background: Color(0xFF1A0F14),
      surface: Color(0xFF2E1A24),
      surfaceLight: Color(0xFF422838),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFFDB2777),
      accentLight: Color(0xFFF472B6),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFFCE7F3),
      textMuted: Color(0xFFD495B8),
      chatBubbleMe: Color(0xFFDB2777),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '石墨灰',
      background: Color(0xFF0F0F11),
      surface: Color(0xFF1C1C1F),
      surfaceLight: Color(0xFF2C2C30),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF6366F1),
      accentLight: Color(0xFF818CF8),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFD4D4D8),
      textMuted: Color(0xFFA1A1AA),
      chatBubbleMe: Color(0xFF6366F1),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '翡翠绿',
      background: Color(0xFF0A1412),
      surface: Color(0xFF0F1F1A),
      surfaceLight: Color(0xFF1A2E26),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF0D9488),
      accentLight: Color(0xFF14B8A6),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFCCFBF1),
      textMuted: Color(0xFF94A3B8),
      chatBubbleMe: Color(0xFF0D9488),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '赛博黄',
      background: Color(0xFF0A0A0A),
      surface: Color(0xFF1A1A1A),
      surfaceLight: Color(0xFF2A2A2A),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFFEAB308),
      accentLight: Color(0xFF84CC16),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF97316),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFFEF9C3),
      textMuted: Color(0xFFA3A3A3),
      chatBubbleMe: Color(0xFFEAB308),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '烈焰红',
      background: Color(0xFF1A0A0A),
      surface: Color(0xFF2E1414),
      surfaceLight: Color(0xFF422020),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFFDC2626),
      accentLight: Color(0xFFEF4444),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFB91C1C),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFFEE2E2),
      textMuted: Color(0xFFA3A3A3),
      chatBubbleMe: Color(0xFFDC2626),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '冰川蓝',
      background: Color(0xFF0A121A),
      surface: Color(0xFF14222E),
      surfaceLight: Color(0xFF1E3442),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF06B6D4),
      accentLight: Color(0xFF22D3EE),
      success: Color(0xFF10B981),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFCFFAFE),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFF06B6D4),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '香槟金',
      background: Color(0xFF14120A),
      surface: Color(0xFF262214),
      surfaceLight: Color(0xFF3A3420),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFFD4A048),
      accentLight: Color(0xFFE8B84C),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFFEF3C7),
      textMuted: Color(0xFFA3A3A3),
      chatBubbleMe: Color(0xFFD4A048),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '罗兰紫',
      background: Color(0xFF120A1A),
      surface: Color(0xFF22142E),
      surfaceLight: Color(0xFF322042),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFFA855F7),
      accentLight: Color(0xFFC084FC),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFF3E8FF),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFFA855F7),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      name: '青竹绿',
      background: Color(0xFF0A1410),
      surface: Color(0xFF14261C),
      surfaceLight: Color(0xFF1E382C),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF65A30D),
      accentLight: Color(0xFF84CC16),
      success: Color(0xFF22C55E),
      warning: Color(0xFFEAB308),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFECFCCB),
      textMuted: Color(0xFF9CA3AF),
      chatBubbleMe: Color(0xFF65A30D),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
    const ThemePreset(
      // 与渌景数字科技 logo 的「深青蓝 + 翠绿」呼应
      name: '珍珠白',
      background: Color(0xFFF4F7F6),       // 极淡青白色，比纯灰白更柔
      surface: Color(0xFFFFFFFF),
      surfaceLight: Color(0xFFF8FBFA),
      glassBg: Color(0x0D14424F),          // 玻璃底色带 logo 深青底
      glassBorder: Color(0x1F14424F),
      divider: Color(0xFFD8E4E5),          // 淡青灰分割线
      accent: Color(0xFF1F6478),           // logo 深青蓝（主色）
      accentLight: Color(0xFF2FA08C),      // logo 翠绿（强调副色）
      success: Color(0xFF22C55E),
      warning: Color(0xFFE4A11B),
      danger: Color(0xFFD83A52),
      textPrimary: Color(0xFF14424F),      // 深青墨色，与 logo「渌景数字科技」字色一致
      textSecondary: Color(0xFF4E6F77),    // 中性青灰，比原暖灰对比度更高
      textMuted: Color(0xFF8AA1A6),        // 浅青灰，比 #B5B0AA 易读
      chatBubbleMe: Color(0xFF1F6478),     // 自己的气泡 = 主色
      chatBubbleOther: Color(0xFFEFF4F4),  // 对方气泡 = 极淡青白
      chatBubbleSign: Color(0xFFDFEEEA),   // 手语识别气泡：浅翠绿
      chatBubbleSpeech: Color(0xFFDDE9EE), // 语音输入气泡：浅青蓝
    ),
    const ThemePreset(
      name: '钨钢灰',
      background: Color(0xFF0E0E10),
      surface: Color(0xFF1A1A1E),
      surfaceLight: Color(0xFF2A2A30),
      glassBg: Color(0x0DFFFFFF),
      glassBorder: Color(0x1AFFFFFF),
      divider: Color(0x33FFFFFF),
      accent: Color(0xFF8B8FA3),
      accentLight: Color(0xFFA8ADBF),
      success: Color(0xFF22C55E),
      warning: Color(0xFFF59E0B),
      danger: Color(0xFFEF4444),
      textPrimary: Color(0xFFFFFFFF),
      textSecondary: Color(0xFFD4D4D8),
      textMuted: Color(0xFF8B8FA3),
      chatBubbleMe: Color(0xFF8B8FA3),
      chatBubbleOther: Color(0x1AFFFFFF),
      chatBubbleSign: Color(0xFF1E3A5F),
      chatBubbleSpeech: Color(0xFF2D1B69),
    ),
  ];
}
